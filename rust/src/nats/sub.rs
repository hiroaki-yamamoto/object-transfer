...
{
    pub async fn new(js: jetstream::Context, format: Format) -> Result<Self, Error> {
        let stream = js
        ...
        let stream = messages.then(move |msg_res| {
            ...
            async move {
                ...
                let data = match format {
                    Format::MessagePack => {
                        rmp_serde::from_slice::<T>(&msg.message.payload).map_err(Error::MessagePackDecode)
                    }
                    Format::JSON => {
                        serde_json::from_slice::<T>(&msg.message.payload).map_err(Error::Json)
                    }
                }?;
                Ok(data)
            }
